...
func TestJaccardSimilarity_IdenticalSets(t *testing.T) {
	...
	sim, common := jaccardSimilarity(set, set)

	if sim != 1.0 {
		t.Errorf("jaccardSimilarity() with identical sets = %v, want 1.0", sim)
	}
	if len(common) != 3 {
		t.Errorf("jaccardSimilarity() common keywords = %d, want 3", len(common))
	}
}

func TestJaccardSimilarity_DisjointSets(t *testing.T) {
	set1 := []string{"apple", "banana"}
	set2 := []string{"cherry", "date"}
	sim, common := jaccardSimilarity(set1, set2)

	if sim != 0.0 {
		t.Errorf("jaccardSimilarity() with disjoint sets = %v, want 0.0", sim)
	}
	if len(common) != 0 {
		t.Errorf("jaccardSimilarity() common keywords = %d, want 0", len(common))
	}
}
...
func TestJaccardSimilarity_PartialOverlap(t *testing.T) {
	...
	if sim != 0.5 {
		t.Errorf("jaccardSimilarity() = %v, want 0.5", sim)
	}
	if len(common) != 2 {
		t.Errorf("jaccardSimilarity() common = %d, want 2", len(common))
	}
}
...
func TestJaccardSimilarity_EmptySet(t *testing.T) {
	tests := []struct {
		...
		{"first empty", []string{}, []string{"a", "b"}, 0.0},
		{"second empty", []string{"a", "b"}, []string{}, 0.0},
		{"both empty", []string{}, []string{}, 0.0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sim, _ := jaccardSimilarity(tt.set1, tt.set2)
			if sim != tt.wantSim {
				t.Errorf("jaccardSimilarity() = %v, want %v", sim, tt.wantSim)
			}
		})
	}
}

func TestJaccardSimilarity_CommonKeywordsSorted(t *testing.T) {
	set1 := []string{"zebra", "apple", "mango"}
	set2 := []string{"apple", "banana", "mango", "zebra"}

	_, common := jaccardSimilarity(set1, set2)

	expected := []string{"apple", "mango", "zebra"}
	if len(common) != len(expected) {
		t.Fatalf("common keywords len = %d, want %d", len(common), len(expected))
	}
	for i, kw := range common {
		if kw != expected[i] {
			t.Errorf("common[%d] = %q, want %q", i, kw, expected[i])
		}
	}
}
